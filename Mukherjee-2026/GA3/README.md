...
- Isolates the target channel (typically the tdTomato channel marking dopaminergic neurons)
- Exports a single-channel TIFF to a designated output directory

**When to run:** After epifluorescence image acquisition (step 1). Must be run before any preprocessing or AI segmentation steps.
...
**Purpose:** Preprocesses deconvolved images and applies a Bright Spot Detection node to isolate neuronal soma signals in preparation for the first round of AI segmentation.
...
- Applies a Rolling Ball Background Subtraction and a Local Contrast node to enhance image contrast
- Applies a composed bright spot detection filter (`SegComposedBrightSpotDetectionImpl`) to enhance punctate or soma-like structures and create a mask (binary) of potential DA neuron somas
- Saves the processed image and the binaries as a new TIFF ready for AI-1 input
...
**Note:** The rolling ball, local contrast and bright spot detection nodes are tuned for the morphology of dopaminergic neuron cell bodies. If applied to a different cell type or marker, the detection parameters may need adjustment inside the GA3 graph editor.
...
- Outputs segmentation binaries for each sample
...
**Output:** AI-1 segmentation binaries `.tif`
...
**Purpose:** Runs a **second AI segmentation pass** (AI-2) to refine or complement the detections from AI-1, trained using manually corrected binaries resulting from AI-1.

**What it does:**

- Loads TIFF files (typically the corrected of AI-1)
- Applies a second `ComposedSegmentObjectsAiImpl` deep learning node with a different model or parameter set
- Produces refined segmentation binaries for the counting step
...
**Output:** AI-2 refined segmentation binaries `.tif`

**Note:** Steps 4 and 6 use the same underlying segmentation node type but are intended to apply different models or handle different object classes (e.g., AI-1 for soma detection, AI-2 for exclusion of false positives and inclusion of false negatives). Confirm model assignment in each file separately before running.
...
- Opens the relevant image (typically the tdTomato-channel TIFF or a merged reference image)
- Prompts the user to draw or load ROI boundaries corresponding to the SNc and VTA
- Saves the ROI definitions (coordinates or binaries) for use in step 8
...
**Input:** Reference image TIFF (tdTomato channel or overlay)  
...
**Purpose:** **Counts dopaminergic (DA) neurons** within the SNc and VTA ROIs defined in step 7, using the AI segmentation binaries from steps 4 or 6.

**What it does:**

- Loads the segmentation binaries and the corresponding ROI definitions
- Restricts object detection to within each anatomical ROI
- Counts the number of segmented objects (tdTomato+ neuronal soma) in the SNc and VTA separately
...
**Input:** AI segmentation binaries from steps 4 or 6 and ROI files from step 7  
...
**Input:** Multi-channel confocal z-stack `.nd2` or `.tif` files containing Iba1 and CD68 channels  
**Output:** Segmentation results and/or measurement tables for Iba1 and CD68

**Note:** The four threshold nodes represent one segmentation pass per channel. Threshold values should be validated against a set of representative images before batch processing.
...
Each file containing a `ComposedSegmentObjectsAiImpl` node (`4-Run_AI-1_on_samples_TIFF.ga3`, `6-Run_AI-2_on_TIFF.ga3`, `Iba1_and_CD68_-_only_EDF.ga3`) requires a pre-trained model to be associated with that node. If GA3s are transferred to a new workstation:
...
| Step 4    | AI-1 segmentation binaries `.tif` per sample                       |
| Step 6    | AI-2 refined segmentation binaries `.tif` per sample               |
| Step 7    | ROI definition files (SNc, VTA) per section                        |
| Step 8    | Cell count table `.csv` (SNc count, VTA count, per section/animal) |
| Iba1/CD68 | Segmentation binaries and measurement table for Iba1 and CD68      |
